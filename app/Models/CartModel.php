...
class CartModel extends BaseModel
{
    ...
    protected $builderMessageBuyerToSeller;
    ...
    public function __construct()
    {
        ...
        $this->builderMessageBuyerToSeller = $this->db->table('message_buyer_to_seller');
    }
    ...
    public function addToCart($product)
    {
        ...
        $giftWrappingFee = inputPost('gift_fee_value');
        if ($quantity < 1) {
            $quantity = 1;
        }
            ...
            foreach ($cart as $item) {
                if ($item->product_id == $productId && $item->product_title == $productTitle) {
                    if ($product->listing_type != 'license_key' && $product->product_type != 'digital') {
                        $item->quantity += 1;
                        if($giftWrappingFee > 0) $item->gift_quantity += 1;
                        else $item->no_gift_quantity += 1;
                    }
                    $updateQuantity = 1;
                }
            }
        ...
        if ($updateQuantity == 1) {
            ...
            $item->no_gift_quantity = $giftWrappingFee > 0 ? 0 : $quantity;
            $item->gift_quantity = $giftWrappingFee > 0 ? $quantity : 0;
            $item->quantity = $item->no_gift_quantity + $item->gift_quantity;
    ...
    public function updateCartProductQuantity($productId, $cartItemId, $quantity)
    {
        ...
        $isGift = inputPost('is_gift');
        $cart = $this->sessionCartItems;
        if (!empty($cart)) {
            foreach ($cart as $item) {
                if ($item->cart_item_id == $cartItemId) {
                    $item->gift_quantity = ($isGift == 1) ? $quantity : $item->gift_quantity;
                    $item->no_gift_quantity = ($isGift == 0) ? $quantity : $item->no_gift_quantity;
                    $item->quantity = $item->gift_quantity + $item->no_gift_quantity;
                }
            }
        }
        helperSetSession('mds_shopping_cart', $cart);
            ...
            foreach ($cart as $cartItem) {
                ...
                if (!empty($product)) {
                    ...
                    if ($cartItem->purchase_type == 'bidding') {
                        ...
                        $item->options_array = $cartItem->options_array;
                        $item->quantity = $cartItem->quantity;
                        $item->gift_quantity = $cartItem->gift_quantity;
                        $item->no_gift_quantity = $cartItem->no_gift_quantity;
                        $item->unit_price = $object->price_calculated;
                        $item->unit_gift_price = $object->price_calculated + getPrice($product->gift_wrapping_fee,'decimal');
                        $item->unit_no_gift_price = $object->price_calculated;
                        $item->total_price = $object->price_calculated * $cartItem->quantity;
                        $item->total_gift_price = $item->unit_gift_price * $item->gift_quantity;
                        $item->total_no_gift_price = $item->unit_no_gift_price * $item->no_gift_quantity;
                        $item->total_gift_wrapping_fee = getPrice($product->gift_wrapping_fee,'decimal') * $item->gift_quantity;
    ...
    //get cart items seller ids
    public function getSessCartItemSellerIds()
    {
        $cart = array();
        $sellerIds = array();
        if (!empty(helperGetSession('mds_shopping_cart'))) {
        ...
        if (!empty($cart)) {
            foreach ($cart as $cartItem) {
                $product = getActiveProduct($cartItem->product_id);
                if (!empty($product)) {
                    if(!in_array($cartItem->seller_id, $sellerIds)) array_push($sellerIds,$cartItem->seller_id);
                }
            }
        }
        return $sellerIds;
    ...
    public function calculateCartTotal($cartItems, $currencyCode = null, $setSession = true)
    {
        ...
        $cartTotal->total = 0;
        $cartTotal->subtotal_gift = 0;
        $cartTotal->subtotal_no_gift = 0;
        $cartTotal->total_gift_wrapping_fee = 0;
        ...
        if (!empty($cartItems)) {
            foreach ($cartItems as $item) {
                if ($item->purchase_type == 'bidding') {
                    ...
                    $cartTotal->subtotal += $item->total_price;
                    $cartTotal->subtotal_gift += $item->total_gift_price;
                    $cartTotal->subtotal_no_gift += $item->total_no_gift_price;
                    $cartTotal->total_gift_wrapping_fee += $item->total_gift_wrapping_fee;
        ...
        $cartTotal->total_before_shipping = $cartTotal->subtotal + $cartTotal->vat + $cartTotal->total_gift_wrapping_fee;
        ...
        foreach ($cart as $cartItem) {
            ...
            if (!empty($product)) {
                ...
                if ($cartItem->purchase_type == 'bidding') {
                    ...
                    $item->no_gift_quantity = $cartItem->no_gift_quantity;
                    $item->unit_price = $object->price_calculated;
                    $item->unit_gift_price = $cartItem->unit_gift_price;
                    $item->unit_no_gift_price = $cartItem->unit_no_gift_price;
                    $item->total_price = $object->price_calculated * $cartItem->quantity;
                    $item->total_gift_price = $item->unit_gift_price * $item->gift_quantity;
                    $item->total_no_gift_price = $item->unit_no_gift_price * $item->no_gift_quantity;
                    $item->total_gift_wrapping_fee = getPrice($product->gift_wrapping_fee,'decimal') * $item->gift_quantity;
    ...
    public function addMessageToSeller(){
        $data = [
          "buyer_id"=>inputPost("buyer_id"),
          "seller_id"=>inputPost("seller_id"),
          "message"=>inputPost("message")
        ];
        $this->builderMessageBuyerToSeller->insert($data);
    }

    //get message from buyer to seller
    public function getMessageFromBuyerToSeller($buyerId, $sellerId)
    {
        return $this->builderMessageBuyerToSeller->where("buyer_id", $buyerId)->where("seller_id", $sellerId)->select("message")->orderBy("id","DESC")->get()->getRow();
    }

    //update message from buyer to seller
    public function updateMessageFromBuyerToSeller($buyerId, $sellerId, $orderNumber){
        $data = [
            "order_number"=>clrNum($orderNumber),
            "updated_at"=>date("Y-m-d H:i:s")
        ];
        return $this->builderMessageBuyerToSeller->where('buyer_id', clrNum($buyerId))->where('seller_id', clrNum($sellerId))->update($data);
    }

    //get messages by seller
    public function getMessagesBySellerId($sellerId){
        return $this->builderMessageBuyerToSeller->where("seller_id", $sellerId)->get()->getResult();
    }

    //is message exist from buyer to seller
    public function isExistMessageFromBuyerToSeller($buyerId, $sellerId){
        return !empty($this->builderMessageBuyerToSeller->where("buyer_id",$buyerId)->where("seller_id",$sellerId)->select("message")->get()->getRow());
    }

    //delete message from buyer
    public function deleteMessageFromBuyer(){
        $id = inputPost("message_id");
        $this->builderMessageBuyerToSeller->where("id",$id)->delete();
    }
}
