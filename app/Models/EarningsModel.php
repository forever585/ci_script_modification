...
//        $orderProduct = $orderModel->getOrderProduct(1);
//        //=====================
        ...
        if (!empty($orderProduct)) {
            ...
            if (!empty($order)) {
                ...
                if ($row < 1) {
                    $saleAmount = getPrice($orderProduct->product_total_gift_contained_price, 'decimal');
                    $giftWrappingFee = getPrice($orderProduct->gift_wrapping_fee * $orderProduct->gift_quantity, 'decimal');
                    $vat = 0;
                    if (!empty($orderProduct->product_vat)) {
                        $vat = getPrice($orderProduct->product_vat, 'decimal');
                    }
                    $saleAmount = $saleAmount - $vat - $giftWrappingFee;
                    ...
                    $earnedAmount = $earnedAmount + $giftWrappingFee;
                    //add earning
                    $data = [
                        ...
                        'total_gift_wrapping_fee'=>$giftWrappingFee,
                    ...
                    if (!empty($user)) {
                        ...
                        $sales = $sales + 1;
                        $authModel->increaseTotalEarn($orderProduct->seller_id, $totalEarned);
                        $data = [
                            'balance' => $newBalance,
