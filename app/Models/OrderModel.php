...
class OrderModel extends BaseModel
{
    ...
    public function addOrder($dataTransaction, $isOfflinePayment, $offlinePaymentMethod = '')
    {
        ...
        if (!empty($cartTotal)) {
            $data = [
                ...
                'total_gift_wrapping_fee' => getPrice($cartTotal->total_gift_wrapping_fee, 'database'),
            ...
            foreach ($cartItems as $cartItem) {
                ...
                if (!empty($product)) {
                    ...
                    $data = [
                        ...
                        'gift_quantity' => $cartItem->gift_quantity,
                        'gift_wrapping_fee' => $cartItem->gift_wrapping_fee,
                        ...
                        'product_total_gift_contained_price' => getPrice($cartItem->total_gift_price + $cartItem->total_no_gift_price + $cartItem->product_vat, 'database'),
                    ...
                    $cartModel->updateMessageFromBuyerToSeller($this->getBuyerIdByOrderId($orderId)->buyer_id, $product->user_id, $this->getOrderNumberByOrderId($orderId)->order_number);
        ...
        if (!empty($orderProduct)) {
            if ($orderProduct->seller_id == user()->id) {
                ...
                if ($data['order_status'] == 'shipped') {
                    ...
                    $createdTime = date_create($orderProduct->created_at);
                    $updatedTime = date_create();
                    if($this->isQuickReply($createdTime,$updatedTime) && $this->builderOrderProducts->select('order_status')->where('id',$orderProduct->id)->get() != 'shipped'){
                        $authModel = new AuthModel();
                        $authModel->increaseNumberOfQuickShipping($orderProduct->seller_id);
                    }
                }
    ...
    public function isQuickReply($createdTime,$updatedTime){
        $diffHours = clrNum(date_diff($updatedTime,$createdTime)->format('%h'));
        $threshold = clrNum($this->generalSettings->threshold_of_quick_shipping);
    ...
    {
        $this->filterSales($status);
        return $this->builder->join('order_products', 'order_products.order_id = orders.id')->select('orders.*, order_products.shipping_tracking_number')->groupBy('orders.id')->where('order_products.seller_id', clrNum($userId))
                ...
                if (!empty($order)) {
                    ...
                    if (!empty($orderProducts)) {
                        foreach ($orderProducts as $orderProduct) {
                            ...
                            if(getProduct($orderProduct->product_id)->is_promoted){
                                $admins = getAdminUser();
                                foreach ($admins as $admin){
                                    $emailData = [
                                        'email_type' => 'featured_product_purchased',
                                        'email_address' => $admin->email,
                                        'email_subject' => "Featured Product Purchased",
                                        'template_path' => 'email/main',
                                        'email_data' => serialize([
                                            'content' => 'Featured Product Purchased'.'</br>'.getUsernameByUserId($orderProduct->seller_id).' purchased a featured product for '.getProduct($orderProduct->product_title).'. Login to admin area to review',
                                            'url' => base_url(),
                                            'buttonText' => trans("see_details")
                                        ])
                                    ];
                                ...
                                foreach ($admins as $admin){
                                    $emailData = [
                                        ...
                                        'template_path' => 'email/main',
                                        'email_data' => serialize([
                                            'content' => 'Purchase made on Handmade Guild'.'</br>'.getUsernameByUserId($orderProduct->seller_id).' has a new sale, login to admin area to view details ',
                                            //'content' => trans("msg_refund_request_update_email"),
                                            'url' => base_url(),
                                            'buttonText' => trans("see_details")
                                        ])
    ...
    public function isShippingTrackingNumberSet($order_id){
        $trackingNumber = $this->builderOrderProducts->select('shipping_tracking_number')->where('order_id',$order_id)->get()->getResult();
        return isset($trackingNumber);
    }

    public function getBuyerIdByOrderId($orderId){
        return $this->builder->where("id",$orderId)->select("buyer_id")->get()->getRow();
    }

    public function getOrderNumberByOrderId($orderId){
        return $this->builder->where("id",$orderId)->select("order_number")->get()->getRow();
    }
}