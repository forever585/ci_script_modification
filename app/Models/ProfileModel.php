...
class ProfileModel extends BaseModel
{
    ...
    protected $builderFavouriteShops;
    ...
    public function __construct()
    {
        ...
        $this->builderFavouriteShops = $this->db->table('favourite_shops');
    }
    ...
    //add or remove favourite user
    public function addRemoveFavouriteUserPost()
    {
        $data = [
            'shop_owner_id' => inputPost('shop_owner_id'),
            'favourite_user_id' => inputPost('favourite_user_id')
        ];
        $favourite_shop = $this->getFavouriteShop($data["shop_owner_id"], $data["favourite_user_id"]);
        if (empty($favourite_shop)) {
            $this->builderFavouriteShops->insert($data);
            if($this->generalSettings->send_email_added_to_favorite){
                $emailData = [
                    'email_type' => 'add_favorite_shop',
                    'email_address' => getUser($data['shop_owner_id'])->email,
                    'email_subject' => "Favorite Shop Added",
                    'template_path' => 'email/main',
                    'email_data' => serialize([
                        'content' => 'Favorite Shop Added'.'</br>'.getUsernameByUserId($data['favourite_user_id']).' add your shop to favorite shop\'s list ',
                        'url' => base_url(),
            ...
            $this->builderFavouriteShops->where('id', $favourite_shop->id)->delete();
            if($this->generalSettings->send_email_added_to_favorite){
                $emailData = [
                    'email_type' => 'remove_favorite_shop',
                    'email_address' => getUser($data['shop_owner_id'])->email,
                    'email_subject' => "Favorite Shop Removed",
                    'template_path' => 'email/main',
                    'email_data' => serialize([
                        'content' => 'Favorite Shop Removed'.'</br>'.getUsernameByUserId($data['favourite_user_id']).' remove your shop from favorite shop\'s list ',
                        'url' => base_url(),
                        'buttonText' => trans("see_details")
                    ])
                ];
    ...
    //is user favourite
    public function isUserFavourite($shop_owner_id, $favourite_user_id)
    {
        if (empty($this->getFavouriteShop($shop_owner_id, $favourite_user_id))) {
            return false;
        }
        return true;
    }
    ...
    //get favourite shop
    public function getFavouriteShop($shop_owner_id, $favourite_user_id)
    {
        return $this->builderFavouriteShops->where('shop_owner_id', clrNum($shop_owner_id))->where('favourite_user_id', clrNum($favourite_user_id))->get()->getRow();
    }

    //get favourite shops
    public function getFavouriteShops($favourite_user_id)
    {
        return $this->builderFavouriteShops->join('users', 'favourite_shops.shop_owner_id = users.id')->select('users.*')->where('favourite_user_id', clrNum($favourite_user_id))->get()->getResult();
    }

    //get favourite shop owners count
    public function getFavouriteShopsCount($favourite_user_id)
    {
        return $this->builderFavouriteShops->join('users', 'favourite_shops.favourite_user_id = users.id')->select('users.*')->where('favourite_user_id', clrNum($favourite_user_id))->countAllResults();
    }
}
