...
class CommonModel extends BaseModel
{
    ...
    public function reportAbuse()
    {
        ...
        if($this->generalSettings->send_email_product_abuse_report){
            $admins = getAdminUser();
            foreach ($admins as $admin){
                $emailData = [
                    'email_type' => 'abuse_report',
                    'email_address' => $admin->email,
                    'email_subject' => "Abuse Report",
                    'template_path' => 'email/main',
                    'email_data' => serialize([
                        'content' => 'Abuse Report'.'</br>'.getUsernameByUserId(user()->id).' reported a product, please login to admin area and review this information ',
                        'url' => base_url(),
    ...
    public function addReview($rating, $productId, $reviewText)
    {
        ...
        if (!empty($data['product_id']) && !empty($data['user_id']) && !empty($data['rating'])) {
            ...
            $user = $productModel->getUserByProduct($productId);
            $authModel->updateRating($user->id, $rating);
        }
    }
