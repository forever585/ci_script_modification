...
{
    protected $builder;
    protected $builderFavoriteShop;

    public function __construct()
    {
        parent::__construct();
        $this->builder = $this->db->table('users');
        $this->builderFavoriteShop = $this->db->table('favourite_shops');
    }
    ...
    {
        $shop_id = clrNum($shop_id);
        return $this->builder->where('id', $shop_id)->where('is_vacation', 1)->get()->getRow();
    }

    //is user vacation
    public  function isShopVacation($shop_id){
        $shop_id = clrNum($shop_id);
    ...
    public  function enableDisableVacation($userId, $is_vacation_true){

        $data = array(
            'is_vacation' => $is_vacation_true
        );
        return $this->builder->where('id',$userId)->update($data);
    }

    //check whether user can award top seller badge or not
    public function checkUserCanAwardTopSellerBadge($userId){
        $user = $this->getUser($userId);
        if($user->total_earn >= $this->generalSettings->min_top_seller_earn)
            return true;
        else return false;
    }

    //check whether user can award top seller badge or not
    public function checkUserCanAwardRatingBadge($userId){
        $user = $this->getUser($userId);
        if($user->avg_rating_val >= $this->generalSettings->min_avg_rating_val && $user->num_of_reviews >= $this->generalSettings->min_num_of_reviews)
            return true;
        else return false;
    }

    //check whether user can award quick reply badge or not
    public function checkUserCanAwardQuickReplyBadge($userId){
        $user = $this->getUser($userId);
        if($user->num_of_quick_reply >= $this->generalSettings->min_num_of_quick_reply)
            return true;
        else return false;
    }

    //check whether user can award quick shipping badge or not
    public function checkUserCanAwardQuickShippingBadge($userId){
        $user = $this->getUser($userId);
        if($user->num_of_quick_shipping >= $this->generalSettings->min_num_of_quick_shipping)
            return true;
        else return false;
    }

    //increase total earn of seller
    public function increaseTotalEarn($userId, $newEarn){
        $user = $this->getUser($userId);
        $updateEarn = $user->total_earn + $newEarn;
        $data = [
            'total_earn'=> $updateEarn
        ];
        $this->builder->where('id',$userId)->update($data);
    }

    //update average rating value and number of reviews
    public function updateRating($userId, $newReviewVal){
        $user = $this->getUser($userId);
        $updateVal = $user->avg_rating_val * $user->num_of_reviews + $newReviewVal;
        $updateCount = $user->num_of_reviews + 1;
        $data = [
            'avg_rating_val'=> $updateVal / $updateCount,
            'num_of_reviews'=> $updateCount,
        ];
        $this->builder->where('id',$userId)->update($data);
    }

    //increase number of quick reply
    public function increaseNumberOfQuickReply($userId){
        $user = $this->getUser($userId);
        $updateCount = $user->num_of_quick_reply + 1;
        $data = [
            'num_of_quick_reply'=> $updateCount,
        ];
        $this->builder->where('id',$userId)->update($data);
    }

    //increase number of quick shipping
    public function increaseNumberOfQuickShipping($userId){
        $user = $this->getUser($userId);
        $updateCount = $user->num_of_quick_shipping + 1;
        $data = [
            'num_of_quick_shipping'=> $updateCount,
        ];
        return $this->builder->where('id',$userId)->update($data);
    }
    ...
    public function getAdminUser(){
        return $this->builder->where('role_id',1)->get()->getResult();
    }

    //get favorite customers by seller
    public function getFavoriteUsers($shopId){
        return $this->builderFavoriteShop->where('shop_owner_id',$shopId)->get()->getResult();
    }

    //decrease seller's balance as amount of listing fee
    public function decreaseListingFee($sellerId){
        $newBalance = $this->getUser($sellerId)->balance;
        $newBalance = $newBalance - $this->generalSettings->listing_fee * 100;
        $this->increaseTotalEarn($sellerId, $this->generalSettings->listing_fee * (-100));
        $data = [
            'balance' => $newBalance,
        ];
        return $this->db->table('users')->where('id', $sellerId)->update($data);
    }

    //increase seller's balance as amount of listing fee
    public function increaseListingFee($sellerId){
        $newBalance = $this->getUser($sellerId)->balance;
        $newBalance = $newBalance + $this->generalSettings->listing_fee * 100;
        $this->increaseTotalEarn($sellerId, $this->generalSettings->listing_fee * (100));
        $data = [
    ...
    public function increaseAdminBalance(){
        $admin = $this->getAdminUser();
        $this->increaseTotalEarn($admin[0]->id, $this->generalSettings->listing_fee * 100);
        return true;
    }
