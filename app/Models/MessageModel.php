...
class MessageModel extends BaseModel
{
    ...
    public function addMessage($conversationId)
    {
        ...
        if (!empty($data['message'])) {
            if ($this->builderMessages->insert($data)) {
                ...
                if(isVendor(getUser($data['receiver_id'])) && isShopVacation($data['receiver_id'])){
                    $emailData = [
                        'email_type' => 'shop_is_on_vacation',
                        'email_address' => user()->email,
                        'email_subject' => "Shop Is On Vacation",
                        'template_path' => 'email/main',
                        'email_data' => serialize([
                            'content' => 'Shop Is On Vacation'.'</br>'.getUsernameByUserId($data['receiver_id']).' is currently on vacation, they will respond when they return. ',
                            'url' => base_url(),
                    ...
                    $data = [
                        'conversation_id' => $conversationId,
                        'sender_id' => getAdminUser()[0]->id,
                        'receiver_id' => $data['sender_id'],
                        'message' => getUser($data['receiver_id'])->username . ' is on vacation.',
                        'is_read' => 0,
                        'deleted_user_id' => 0,
                        'created_at' => date("Y-m-d H:i:s")
                    ];
                    return $this->builderMessages->insert($data);
                }

                $this->addMessageEmail($messageId);

                //check this is first reply?
                if(isVendor(user()) && !isAdmin()){
                    $receivedMessage = $this->builderMessages->select()->where('conversation_id', $conversationId)->where('receiver_id',user()->id)->get()->getResult();
                    $sentMessage = $this->builderMessages->select()->where('conversation_id', $conversationId)->where('sender_id',user()->id)->get()->getResult();
                    $sendTime = $receivedMessage[0]->created_at;
                    $receiveTime = $sentMessage[0]->created_at;
                    if(dateDifferenceInHours($sendTime, $receiveTime) < clrNum($this->generalSettings->threshold_of_quick_reply))
                        return $authModel->increaseNumberOfQuickReply(user()->id);
                }
                return $messageId;
            }
        }
    ...
    public function addMessageEmail($messageId)
    {
        ...
        if (!empty($message)) {
            ...
            if (!empty($conversation) && !empty($receiver) && $this->generalSettings->send_email_message_email == 1 && !empty($message->message)) {
                $emailData = [
