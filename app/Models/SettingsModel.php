...
class SettingsModel extends BaseModel
{
    ...
    public function __construct()
    {
        ...
        $this->builderBadges = $this->db->table('badges');
    }
    ...
    public function updateEmailOptions()
    {
        $data = [
            ...
            'send_email_refund_request' => inputPost('send_email_refund_request'),
            'send_email_payout_request' => inputPost('send_email_payout_request'),
            'send_email_new_review_set' => inputPost('send_email_new_review_set'),
            'send_email_membership_plan_change' => inputPost('send_email_membership_plan_change'),
            'send_email_purchase_made' => inputPost('send_email_purchase_made'),
            'send_email_featured_product_purchased' => inputPost('send_email_featured_product_purchased'),
            'send_email_product_abuse_report' => inputPost('send_email_product_abuse_report'),
            'send_email_support_ticket' => inputPost('send_email_support_ticket'),
            'send_email_newsletter' => inputPost('send_email_newsletter'),
            'send_email_contact_with_contact_us_form' => inputPost('send_email_contact_with_contact_us_form'),
            'send_email_vacation_mode_notification' => inputPost('send_email_vacation_mode_notification'),
            'send_email_message_email' => inputPost('send_email_message_email'),
            'send_email_added_to_favorite' => inputPost('send_email_added_to_favorite')
        ];
        return $this->builderGeneral->where('id', 1)->update($data);
    }
    ...
    public function updatePreferences($form)
    {
        if ($form == 'homepage') {
            ...
            $data = [
                'min_top_seller_earn' => inputPost('min_top_seller_earn'),
                'min_num_of_quick_reply' => inputPost('min_num_of_quick_reply'),
                'min_avg_rating_val' => inputPost('min_avg_rating_val'),
                'threshold_of_quick_reply' => inputPost('threshold_of_quick_reply'),
                'min_num_of_reviews' => inputPost('min_num_of_reviews'),
                'threshold_of_quick_shipping' =>inputPost('threshold_of_quick_shipping'),
                'min_num_of_quick_shipping' => inputPost('min_num_of_quick_shipping')
            ];
        }
        return $this->builderGeneral->where('id', 1)->update($data);
    }
    ...
    public function updateSystemSettings()
    {
        ...
        }else if($submit == "fee_setting"){
            $isFeeEnable = inputPost("is_fee_enable");
            $listingFee = inputPost("listing_fee");
            if(!empty($listingFee)){
                $data = [
                    "is_fee_enable" => clrNum($isFeeEnable),
    ...
    public function getBadgeNames(){
        return $this->builderBadges->get()->getResult();
    }
}