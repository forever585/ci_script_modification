...
class MembershipModel extends BaseModel
{
        ...
        if ($this->generalSettings->send_email_shop_opening_request == 1) {
            $emailData = [
                ...
                'email_data' => serialize([
                    'content' => trans("there_is_shop_opening_request") . '<br>' . trans("user") . ': ' . '<strong>' . getUsername(user()) . ' ,'.user()->email.'has requested to open a store on Handmade Guild!'.'</strong>'.'<p>please login to admin to view and approve or reject</p>',
    ...
    public function getAllMembershipPlans(){
        return $this->builderMembershipPlans->select()->get()->getResult();
    }

    //get all membership titles
    public function getAllMembershipTitles(){
        $titles = array();
        $plans = $this->getAllMembershipPlans();
        foreach ($plans as $plan){
            $titles[] = [
                "title"=>getMembershipPlanName($plan->title_array, selectedLangId())
            ];
