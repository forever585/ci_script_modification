...
class ProductModel extends BaseModel
{
    ...
    public function editProductDetails($id)
    {
        ...
        $data = [
            ...
            'is_draft' => 0,
            'gift_wrapping_fee'=>inputPost('gift_wrapping_fee')
        ];
        $data['price'] = getPrice($data['price'], 'database');
        $data['gift_wrapping_fee'] = getPrice($data['gift_wrapping_fee'], 'database');
        if (empty($data['price'])) {
            $data['price'] = 0;
        }
        ...
        if (empty($data['gift_wrapping_fee'])) {
            $data['gift_wrapping_fee'] = 0;
        }
    ...
    public function getUserByProduct($productId){
        $authModel = new AuthModel();
