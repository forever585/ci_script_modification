    ...
    <div class="box-body">
            ...
            <div class="col-sm-12">
                <div class="table-responsive">
                    <table class="table table-striped" role="grid">
                        ...
                        <tbody>
                        <?php $messages = getMessagesBySellerId(user()->id);
                        ?>
                        ...
                        <?php if (!empty($messages)): ?>
                            <?php foreach ($messages as $message):
                                if (!empty($message)):?>
                                    <tr>
                                        <td><?= empty($message->order_number) ? "Not Purchased" : "#".$message->order_number; ?></td>
                                        <td><?= getUsernameByUserId($message->buyer_id); ?></td>
                                        <td><?= $message->message; ?></td>
                                        <td><?= $message->created_at; ?></td>
                                        <td><?= empty($message->updated_at) ? "Not Purchased" : $message->updated_at; ?></td>
                                        <td>
                                            <form action="<?= base_url('delete-message-from-buyer'); ?>" method="post">
                                                <?= csrf_field(); ?>
                                                <input type="hidden" name="message_id" value="<?php echo $message->id; ?>"/>
                                                <input type="hidden" name="back_url" value="<?= dashboardUrl('sales') ?>">
                                                <button type="submit" class="btn btn-custom"><i class="fa fa-remove"></i><?= "Delete" ?></button>
                                            </form>
                                        </td>
                                    </tr>
                                <?php endif;
        ...
        <div class="row">
            <div class="col-sm-12">
                <?php if (!empty($messages)): ?>
                    <div class="number-of-entries">
                        <span><?= trans("number_of_entries"); ?>:</span>&nbsp;&nbsp;<strong><?= count($messages); ?></strong>
                    </div>
                <?php endif; ?>
                <div class="table-pagination">
                    <?= view('partials/_pagination'); ?>
