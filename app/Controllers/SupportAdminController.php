...
class SupportAdminController extends BaseAdminController
{
    ...
    public function sendMessagePost()
    {
        ...
        if ($this->supportModel->addSubTicket($ticketId, true)) {
            ...
            if($this->generalSettings->send_email_support_ticket == 1){
                $supportModel = new SupportModel();
                $user = getUser($supportModel->getTicket($ticketId)->user_id);
                $emailData = [
                    'email_type' => 'support_ticket_reply',
                    'email_address' => $user->email,
                    'email_subject' => "Support Ticket Replied",
                    'template_path' => 'email/main',
                    'email_data' => serialize([
                        'content' => 'Support Ticket Reply'.'</br> Handmade Guild has responded to your support ticket, please visit '.base_url().' to view',
                        'url' => base_url(),
