...
class HomeController extends BaseController
{
    ...
    public function renewMembershipPlanPost()
    {
        ...
        if($this->generalSettings->send_email_membership_plan_change == 1){
            $admins = getAdminUser();
            foreach ($admins as $admin) {
                $emailData = [
                    'email_type' => 'membership_plan_changed',
                    'email_address' => $admin->email,
                    'email_subject' => "Membership Plan Changed",
                    'template_path' => 'email/main',
                    'email_data' => serialize([
                        'content' => 'Membership Plan Changed' . '</br>' . user()->username . ' has changed their membership plan, please login to admin to review these changes. ',
                        'url' => base_url(),
            ...
            if (!empty($product)) {
                ...
                if ($addReview) {
                    ...
                    $emailData = [
                        ...
                        'email_address' => getUser($product->user_id)->email,
                        'email_subject' => "New Review Set",
                        'template_path' => 'email/main',
                        'email_data' => serialize([
                            'content' => 'New Review Set '.'</br> There has been a new review posted for '.getProductDetails($productId,1)->title.' sold by you',
                            'url' => base_url(),
                            'buttonText' => trans("see_details")
                        ])
                    ];
                    ...
                    foreach ($admins as $admin){
                        $emailData = [
                            ...
                            'template_path' => 'email/main',
                            'email_data' => serialize([
                                'content' => 'New Review Set '.'</br> There has been a new review posted for '.getProductDetails($productId,1)->title.' sold by '.getUsernameByUserId($product->user_id),
     ...
     * Invoice Listing Fee
     */
    public function invoiceListingFee($id)
    {
        $data['title'] = trans("invoice");
        $data['description'] = trans("invoice") . ' - ' . $this->baseVars->appName;
        $data['keywords'] = trans("invoice") . ',' . $this->baseVars->appName;
        ...
        $data['transaction'] = $feeModel->getListingFee($id);
        if (empty($data['transaction'])) {
            return redirect()->to(langBaseUrl());
        }
        if (!hasPermission('products')) {
            if (user()->id != $data['transaction']->seller_id) {
                return redirect()->to(langBaseUrl());
            }
        }
        $data['currency'] = 1;
        $data['user'] = getUser($data['transaction']->seller_id);
        $data['listing_fee'] = $this->generalSettings->listing_fee;
        $data['transaction_fee'] = calculateTransactionFee($this->generalSettings->listing_fee);
        if (empty($data['user'])) {
            return redirect()->to(langBaseUrl());
        }
        echo view('invoice/invoice_listing_fee', $data);
    }
