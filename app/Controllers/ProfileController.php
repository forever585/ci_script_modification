...
class ProfileController extends BaseController
{
    ...
    public function addRemoveFavouriteUserPost()
    {
        ...
        $this->profileModel->addRemoveFavouriteUserPost();
        redirectToBackUrl();
    }
     ...
     * Get Favourtie Shops
     */
    public function favouriteShops($slug)
    {
        if (!authCheck()) {
            return redirect()->to(langBaseUrl());
        }
        ...
        $data['title'] = "Favourite Shops";
        $data['description'] = "Favourite Shops" . " - " . $this->baseVars->appName;
        $data['keywords'] = "Favourite Shops" . "," . $this->baseVars->appName;

        $data['userSession'] = getUserSession();
        $data["favouriteShops"] = $this->profileModel->getFavouriteShops($data["user"]->id);
        $data["favouriteShopsCount"] = $this->profileModel->getFavouriteShopsCount($data["user"]->id);

        echo view('partials/_header', $data);
        echo view('profile/favourite_shops', $data);
        echo view('partials/_footer');
    }
}