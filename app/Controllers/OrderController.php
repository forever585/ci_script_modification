...
class OrderController extends BaseController
{
    ...
    public function submitRefundRequest()
    {
        ...
        if (!empty($orderProduct)) {
            ...
            if (!empty($this->generalSettings->mail_username) && !empty($user) && !empty($refundId) && $this->generalSettings->send_email_refund_request == 1) {
                $emailData = [
                    'email_type' => 'refund',
                ...
                foreach ($admins as $admin){
                    $emailData = [
                        ...
                        'template_path' => 'email/main',
                        'email_data' => serialize([
                            'content' => trans("msg_refund_request_update_email").'</br>'.getUsernameByUserId($orderProduct->buyer_id).' requests a refund for '.getOrder($orderProduct->order_id)->order_number.' from '.getUsernameByUserId($orderProduct->seller_id),
                            'url' => generateDashUrl("refund_requests") . '/' . $refundId,
                            'buttonText' => trans("see_details")
                        ])
                    ];
            ...
            if (!empty($mailUserId)) {
                ...
                if (!empty($this->generalSettings->mail_username) && !empty($user)) {
                    $emailData = [
                        ...
                        'template_path' => 'email/main',
                        'email_data' => serialize([
                            'content' => trans("msg_refund_request_update_email").'</br>'.getUsernameByUserId($request->buyer_id).' requests a refund for '.getOrder($request->order_id)->order_number.' from '.getUsernameByUserId($request->seller_id),
