...
class DashboardController extends BaseController
{
    ...
    public function withdrawMoneyPost()
    {
        ...
        if ($this->earningsModel->withdrawMoney($data)) {
            ...
            if($this->generalSettings->send_email_payout_request == 1){
                $admins = getAdminUser();
                foreach ($admins as $admin){
                    $emailData = [
                        'email_type' => 'payout_request',
                        'email_address' => $admin->email,
                        'email_subject' => "Payout Requested",
                        'template_path' => 'email/main',
                        'email_data' => serialize([
                            'content' => 'Payout Request Email'.'</br>'.getUsernameByUserId($this->userId).' is requesting a payout for '.priceDecimal($data['amount'],$data['currency']).' please login to admin to approve or reject this payout request ',
     ...
     */
    public function enableDisableVacationMode()
    {
        $is_vacation_true = inputPost('state');
        $shop_id = inputPost('shop_id');
        $id = clrNum($shop_id);
        $this->authModel->enableDisableVacation($id, $is_vacation_true);
        redirectToBackUrl();
    }
    ...
    //listing fees
    public function listingFees(){
        if (!$this->baseVars->isSaleActive) {
            return redirect()->to(dashboardUrl());
        }
        $data = $this->setMetaData(trans("listing-fees"));
        $data['numRows'] = $this->feeModel->getListingFeesCount($this->userId);
        $pager = paginate($this->perPage, $data['numRows']);
        $data['listing_fees'] = $this->feeModel->getListingFeesPaginated($this->userId, $this->perPage, $pager->offset);

        echo view('dashboard/includes/_header', $data);
        echo view('dashboard/earnings/listing_fees', $data);
        echo view('dashboard/includes/_footer');
    }
}
