...
class AuthController extends BaseController
{
    ...
    public function loginPost()
    {
        ...
        if (!$this->validate(getValRules($val))) {
            ...
            $userId = $this->authModel->getUserByEmail(inputPost('email'))->id;
            if(!$this->chargeListingFees($userId)){
                $data = [
                    'result' => 0,
                    'errorMessage' => view('partials/_messages')
                ];
                echo json_encode($this->chargeListingFees($userId));
                return;
            }
            if ($this->authModel->login()) {
                $this->checkAndAwardBadge($userId);
            } else {
                $data = [
                    'result' => 0,
                    'errorMessage' => view('partials/_messages')
    ...
    /**
     * Check User Can Award Badge and if true Award Email
     */
    public function checkAndAwardBadge($userId){
        if(checkUserCanAwardTopSellerBadge($userId))
            $this->badgeModel->awardBadge($userId,1);
        else $this->badgeModel->removeBadge($userId,1);
        if(checkUserCanAwardRatingBadge($userId))
            $this->badgeModel->awardBadge($userId,2);
        else $this->badgeModel->removeBadge($userId,2);
        if(checkUserCanAwardQuickReplyBadge($userId))
            $this->badgeModel->awardBadge($userId,4);
        else $this->badgeModel->removeBadge($userId,4);
        if(checkUserCanAwardQuickShippingBadge($userId))
            $this->badgeModel->awardBadge($userId,5);
        else $this->badgeModel->removeBadge($userId,5);
    }
    ...
    public function chargeListingFees($userId){
        ...
        $result = false;
        if(count($unchargedFees) == 0) $result = true;
        else{
            foreach ($unchargedFees as $unchargedFee){
                if( clrNum(getUser($userId)->balance) >= $this->generalSettings->listing_fee * 100 && $authModel->decreaseListingFee($userId) && $authModel->increaseAdminBalance()){
                    if($feeModel->chargeListingFee($unchargedFee->id))
                        $result = true;
                    else $result = false;
                }
                    ...
                    if($authModel->decreaseListingFee($userId) && $authModel->increaseAdminBalance())
                        $result = true;
                    else $result = false;
                }
                else $result = true;
